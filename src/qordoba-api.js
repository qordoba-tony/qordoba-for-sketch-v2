...
export const loginWithUsernameAndPassword = (email, password, context, browserWindow, webContents) => {
...
const getTokenFromServer = (email, password, context, browserWindow, webContents) => {
	  ...
	  .then((res) => {
	    if (res.status === 200) {
	      res.json().then(data => {
	        ...
	        const userName = data.loggedUser.name;
	        const userID = data.loggedUser.id;
	        const userEmail = data.loggedUser.email;
	        const organizations = data.loggedUser.organizations;
	        utils.saveActiveTokenToComputer(context, token, userID, userName, userEmail, webContents);

	        webContents.executeJavaScript(`logInfoToRollbar("${userName}", "Login successful")`);
	        browserWindow.close();
	      });
	    } else {
